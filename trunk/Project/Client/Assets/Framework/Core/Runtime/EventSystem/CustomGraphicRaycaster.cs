...
namespace Game.Core
{
    ...
    public class CustomGraphicRaycaster : BaseRaycaster
    {
        ...
        public bool onlyScanTargetGraphic = false;

        public bool isEasyTouchDragIgnore = false;

        public static bool isLoadFromPrefab = false;
        ...
        public override void Raycast(PointerEventData eventData, List<RaycastResult> resultAppendList)
        {
            ...
            DoRaycast(canvas, eventCamera, eventPosition, m_RaycastResults);
        ...
        public override Camera eventCamera
        {
            get
            {
                if((object)mCachedEventCamera == null)
                {
                    mCachedEventCamera = canvas.worldCamera;
                }
                return mCachedEventCamera;
            }
        }

        Camera mCachedEventCamera = null;
        /// <summary>
        /// Perform a raycast into the screen and collect all graphics underneath it.
        /// </summary>
        [NonSerialized]
        static readonly List<Graphic> s_SortedGraphics = new List<Graphic>();
        void DoRaycast(Canvas canvas, Camera eventCamera, Vector2 pointerPosition, List<Graphic> results)
        {
            IList<Graphic> foundGraphics = null;
            bool isOnlyScanTargetGraphic = onlyScanTargetGraphic;
#if UNITY_EDITOR
            if(GameConfig.LoadFromPrefab)
            {
                isOnlyScanTargetGraphic = false;
            }
#endif
            if (isOnlyScanTargetGraphic)
            {
                foundGraphics = RaycastTargetRegistry.GetGraphicsForCanvas(canvas);
            }
            else
            {
                foundGraphics = GraphicRegistry.GetGraphicsForCanvas(canvas);
            }
            
            if(foundGraphics.Count == 0)
            {
            ...
            for (int i = 0; i < foundGraphics.Count; ++i)
            {
                ...
                if (float.IsNaN(pointerPosition.x) || float.IsNaN(pointerPosition.y)|| float.IsInfinity(pointerPosition.x) || float.IsInfinity(pointerPosition.y))
                    continue;
