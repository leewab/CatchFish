...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class DepthOfField : PostEffectsBase {
        ...
        public static bool ISDOF = false;
        //private short NextFrameReloadUI = 0;
        ...
        public bool IsNearBlur = true;//是否近景模糊
        public bool IsFarBlur = true;//是否远景模糊
        public bool IsAutoComputeTarget = false;
        public bool IsAllowManageCameraDepthMode = false;
        ...
        {
            get { return focalTransform; }
            set
            {
                if (isFocalTransformLocked)
                {
                    hasCachedFocalTransform = true;
                    cachedFocalTransform = value;
                }
                else
                {
        ...
        [SerializeField]
        private Transform focalTransform = null;

        private bool isFocalTransformLocked = false;
        private Transform cachedFocalTransform;
        private bool hasCachedFocalTransform = false;

        /// <summary>
        /// 锁定聚焦对象
        /// </summary>
        /// <param name="target">聚焦对象</param>
        public void LockFocalTransform(Transform target)
        {
            isFocalTransformLocked = true;
            focalTransform = target;
            hasCachedFocalTransform = false;
        }

        /// <summary>
        /// 取消聚焦对象的锁定
        /// </summary>
        public void UnlockFocalTransform()
        {
            if (hasCachedFocalTransform)
            {
                focalTransform = cachedFocalTransform;
            }

            isFocalTransformLocked = false;
            hasCachedFocalTransform = false;
            cachedFocalTransform = null;
        }
        ...
        void OnEnable() {
            ...
            if (IsAllowManageCameraDepthMode)
            {
                if (cachedCamera != null)
                {
                    cachedCamera.depthTextureMode = DepthTextureMode.Depth;
                }
                //EngineFeatureMgr.Set(true, DepthTextureMode.Depth, "STD Base : " + gameObject.name);
        ...
        void OnDisable() {
            ...
            if (IsAllowManageCameraDepthMode)
            {
                if (cachedCamera != null)
                {
                    cachedCamera.depthTextureMode = DepthTextureMode.None;
                }
                //EngineFeatureMgr.Set(false, DepthTextureMode.Depth, "STD Base : " + gameObject.name);
            }
        ...
        private bool CheckRenderer(Renderer r) {

            try {
                string shader = r.sharedMaterial.shader.name;
                if (!shader.ToLower().Contains("forward")) return false;
            } catch {
                return false;
            }
            return true;
        ...
        private float GetMaxFocusDistance(Transform root) {

            //return cachedCamera.WorldToViewportPoint(root.position).z;

            if (!IsAutoComputeTarget) {
                return cachedCamera.WorldToViewportPoint(root.position).z;
            }
            ...
            foreach (Renderer child in renders) {
                if (CheckRenderer(child)) {
                    center += child.bounds.center - postion;
                    num++;
                }
            }
            ...
            Bounds bounds = new Bounds(center + postion, Vector3.zero);
            foreach (Renderer child in renders) {
                if (CheckRenderer(child)) {
            ...
            center = bounds.center;// + parent.position;//包围盒中心
            float height = bounds.size.y;//包围盒高度
            float width = bounds.size.x;
            float length = bounds.size.z;

            Vector3 point = center;

            Vector3 point_a = center + new Vector3(width / 2, height / 2, length / 2);
            Vector3 point_b = center + new Vector3(width / 2, height / 2, -length / 2);
            Vector3 point_c = center + new Vector3(width / 2, -height / 2, length / 2);
            Vector3 point_d = center + new Vector3(width / 2, -height / 2, -length / 2);
            Vector3 point_e = center + new Vector3(-width / 2, height / 2, length / 2);
            Vector3 point_f = center + new Vector3(-width / 2, height / 2, -length / 2);
            Vector3 point_g = center + new Vector3(-width / 2, -height / 2, length / 2);
            Vector3 point_h = center + new Vector3(-width / 2, -height / 2, -length / 2);

            float distance_ans = cachedCamera.WorldToViewportPoint(point_a).z;
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_b).z);
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_c).z);
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_d).z);
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_e).z);
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_f).z);
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_g).z);
            distance_ans = Mathf.Max(distance_ans, cachedCamera.WorldToViewportPoint(point_h).z);

            return distance_ans;
        }

        //Material UIDOFMat = null;
        //Material defaultMat = null;
        //List<GameObject> rubbish = null;
        //List<GameObject> HP = null;

        //private void CreateUI() {
        //    GameObject root = GameObject.Find("UI_HeadContainer");
        //    if (root == null) return;
        //    if (UIDOFMat == null) {
        //        Shader shader = Shader.Find("UI/DefaultDOF");
        //        if (shader != null) {
        //            UIDOFMat = new Material(shader);
        //        }
        //    }
        //    if (UIDOFMat == null) return;

        //    rubbish = new List<GameObject>();
        //    HP = new List<GameObject>();

        //    CanvasRenderer[] renderers = root.GetComponentsInChildren<CanvasRenderer>();

        //    for (int i = 0; i < renderers.Length; i++) {
        //        if (renderers[i] == null) continue;
        //        Material mat = renderers[i].GetMaterial();
        //        if (mat != null) {
        //            if (mat.shader.name == "UI/Default_NoGlow") {
        //                defaultMat = mat;

        //                if (renderers[i].gameObject.name == "Fill" || renderers[i].gameObject.name == "FillArea") {
        //                    HP.Add(renderers[i].gameObject);
        //                    renderers[i].gameObject.SetActive(false);
        //                    continue;
        //                }

        //                GameObject DOFObj = GameObject.Instantiate<GameObject>(renderers[i].gameObject);
        //                rubbish.Add(DOFObj);

        //                while (DOFObj.transform.childCount > 0) {
        //                    GameObject.DestroyImmediate(DOFObj.transform.GetChild(0).gameObject);
        //                }

        //                DOFObj.name = "UI_WriteDepth";
        //                DOFObj.transform.parent = renderers[i].transform;
        //                DOFObj.transform.localPosition = Vector3.zero;
        //                DOFObj.transform.localRotation = Quaternion.identity;
        //                DOFObj.transform.localScale = Vector3.one;
                        
        //                renderers[i].SetMaterial(UIDOFMat, 0);
        //            }
        //        }
        //    }
        //}
        ...
        //            if (rubbish[i] != null) {
        //                GameObject.DestroyImmediate(rubbish[i]);
        //            }
        //        }
        //        rubbish.Clear();
        //        rubbish = null;
        ...
        //            Material mat = renderers[i].GetMaterial();
        //            if (mat != null) {
        //                if (mat.shader.name == "UI/DefaultDOF") {
        //                    renderers[i].SetMaterial(defaultMat, 0);
        //                }
        //            }
        //        }
        //    }
        ...
        //    if (UIDOFMat != null) {
        //        Material.DestroyImmediate(UIDOFMat, true);
        //    }
        //    defaultMat = null;
        //}
        ...
        //private void RealReloadUI() {

        //    if (NextFrameReloadUI == 0) return;
        //    NextFrameReloadUI++;
        //    if (NextFrameReloadUI == 3) {
        //        NextFrameReloadUI = 0;
        //        ClearUI();
        //        if (isUiDOF) {
        //            CreateUI();
        ...
        private void Update() {
            ...
            // int v = (int)LightFaceEffect.GetQuality();
            //
            // if (v >= (int)TerrainXQualityLevel.Mid) {
            //     this.enabled = false;
            //     return;
            // }
        }
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            maxBlurSize = Mathf.Min(maxBlurSize, 2.0f);
            focalSize = Mathf.Clamp(focalSize, 0.0f, 2.0f);
            internalBlurWidth = Mathf.Max(maxBlurSize, 0.0f);

            // focal & coc calculations

            focalDistance01 = (focalTransform) ? GetMaxFocusDistance(focalTransform) / (cachedCamera.farClipPlane) : FocalDistance01(focalLength);
            dofHdrMaterial.SetVector("_CurveParams", new Vector4(1.0f, focalSize, (1.0f / (1.0f - aperture) - 1.0f), focalDistance01));
            dofHdrMaterial.SetInt("_IsFar", IsFarBlur ? 1 : 0);
            dofHdrMaterial.SetInt("_IsNear", IsNearBlur ? 1 : 0);
