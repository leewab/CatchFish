...
namespace HedgehogTeam.EasyTouch
{
    ...
    public class EasyTouch : MonoBehaviour
    {
        ...
        public delegate void UIElementTouchDownHanlder(Gesture gesture);
        #endregion
        ...
        public static event UIElementTouchDownHanlder On_UIElementTouchDown;
        #endregion
        ...
        public enum EvtType
        {
            ...
            On_Drag2Fingers, On_DragEnd2Fingers, On_SwipeStart2Fingers, On_Swipe2Fingers, On_SwipeEnd2Fingers, On_EasyTouchIsReady, On_Cancel, On_Cancel2Fingers, On_OverUIElement, On_UIElementTouchUp, On_UIElementTouchDown
        }
        ...
        private Gesture _currentGesture = Gesture.Alloc();
        ...
        private Finger[] fingers = new Finger[MAX_FINGER_NUM];                  // The informations of the touch for finger 1.
        ...
        private DoubleTap[] singleDoubleTap = new DoubleTap[MAX_FINGER_NUM];
        private Finger[] tmpArray = new Finger[MAX_FINGER_NUM];
        ...
        void Start()
        {
            ...
            // Current gesture
            _currentGestures.Add(Gesture.Alloc());
        }
        ...
        void Update()
        {
            ...
            if (enable && EasyTouch.instance == this)
            {
                ...
                //	enableRemote = false;
                //}
                enableRemote = Application.isPlaying && Input.touchCount > 0;
#else
            enableRemote = false;
#endif
        ...
        void LateUpdate()
        {
            ...
            // single gesture
            //_currentGesture在目前神魔项目中没有用到，所以先注释掉
            /* if (_currentGestures.Count > 1)
             {
                 _currentGestures[0].Free();
                 _currentGestures.RemoveAt(0);
             }
             else
             {
                 _currentGestures[0] = Gesture.Alloc(); //这里会导致每帧new Gesture(),所以先注释掉
             }
        ...
        void UpdateTouches(bool realTouch, int touchCount)
        {
            ...
            if (realTouch || enableRemote)
            {
                ...
                //【13852】【摇杆】EasyTouch.Update
                for (var i = 0; i < touchCount && i < MAX_FINGER_NUM; ++i)
                {
                    Touch touch = Input.GetTouch(i);

                    int t = 0;
                    while (t < MAX_FINGER_NUM && fingers[i] == null)
                    {
                        if (tmpArray[t] != null)
                        {
            ...
            {
                int i = 0;
                while (i < touchCount && i < MAX_FINGER_NUM)
                {
                    fingers[i] = input.GetMouseTouch(i, tmpArray[i]) as Finger;
                    if (fingers[i] != null)
                    {
                        fingers[i].touchCount = touchCount;
                    }
        ...
        private void CreateGesture(int touchIndex, EvtType message, Finger finger, SwipeDirection swipe, float swipeLength, Vector2 swipeVector)
        {
            ...
            {
                firingEvent = !finger.isOverGui || IsIgnoreUIEvent(message);
            }
            ...
            else if (finger.isOverGui)
            {
                ...
                else if (message == EvtType.On_TouchStart)
                {
                    RaiseEvent(EvtType.On_UIElementTouchDown, gesture);
                }
                else
                {
        ...
        //在原来EasyTouch的设计中，开启enableUIMode的情况下，如果点击到了UI，就不会触发除了少数UI特殊事件之外的其它事件
        //现在有需求：如果点击到了某些特殊的UI（主界面的UI），拖动事件需要正常触发。（这样，即使点击到了主界面UI，仍然可以拖动摄像机）
        //该函数依赖于uiRaycastResultCache中的数据，所以必须保证本帧已经进行了UI检测，才能使用该函数
        private bool IsIgnoreUIEvent(EvtType evtType)
        {
            if (evtType != EvtType.On_Swipe && evtType != EvtType.On_SwipeStart && evtType != EvtType.On_SwipeEnd)
            {
                return false;
            }
            for (int i = 0; i < uiRaycastResultCache.Count; i++)
            {
                var raycastResult = uiRaycastResultCache[i];
                if (!IsIgnoreRaycastResult(raycastResult))
                {
                    return false;
                }
            }
        ...
        private bool IsIgnoreRaycastResult(RaycastResult result)
        {
            //return (result.module is GOGUI.CustomGraphicRaycaster) && result.gameObject != null && result.gameObject.CompareTag("ClickOver");
            CustomGraphicRaycaster raycaster = result.module as CustomGraphicRaycaster;
            if (raycaster != null && raycaster.isEasyTouchDragIgnore)
            {
                return true;
            }
            if (InputController.IsEasyTouchDragIgnoreGameObject(result.gameObject))
            {
                return true;
            }
            return false;
        ...
        private void TwoFinger()
        {
            ...
            if (twoFinger.currentGesture == GestureType.None)
            {
                ...
                if (!singleDoubleTap[MAX_FINGER_NUM - 1].inDoubleTap)
                {
                    singleDoubleTap[MAX_FINGER_NUM - 1].inDoubleTap = true;
                    singleDoubleTap[MAX_FINGER_NUM - 1].time = 0;
                    singleDoubleTap[MAX_FINGER_NUM - 1].count = 1;
                }
            ...
            if (singleDoubleTap[MAX_FINGER_NUM - 1].inDoubleTap) singleDoubleTap[MAX_FINGER_NUM - 1].time += Time.deltaTime;
            ...
            switch (gesture)
            {
                ...
                case GestureType.Acquisition:
                    ...
                    if (doubleTapDetection == DoubleTapDetection.BySystem)
                    {
                        ...
                        twoFinger.swipeStart = false;
                        twoFinger.dragStart = false;
                        singleDoubleTap[MAX_FINGER_NUM - 1].Stop();
                        StopCoroutine("SingleOrDouble2Fingers");

                    }
                    else
                    {
                        if (!singleDoubleTap[MAX_FINGER_NUM - 1].inWait)
                        {
                            StartCoroutine("SingleOrDouble2Fingers");
                        }
                        else
                        {
                            singleDoubleTap[MAX_FINGER_NUM - 1].count++;
                        }
                    }
                    break;
        ...
        {
            singleDoubleTap[MAX_FINGER_NUM - 1].inWait = true;

            yield return new WaitForSeconds(doubleTapTime);

            if (singleDoubleTap[MAX_FINGER_NUM - 1].count < 2)
            {
                                           ...
                                           float actionTime, SwipeDirection swipe, float swipeLength, Vector2 swipeVector, float twist, float pinch, float twoDistance)
        {
            ...
            else if (gesture.isOverGui)
            {
                ...
                else if (message == EvtType.On_Pinch ||
                    message == EvtType.On_PinchIn ||
                    message == EvtType.On_PinchOut ||
                    message == EvtType.On_PinchEnd)
                {
        ...
        private void RaiseEvent(EvtType evnt, Gesture gesture)
        {
            ...
            if (UnityEngine.Profiling.Profiler.enabled)
                UnityEngine.Profiling.Profiler.BeginSample("easy touch raise event : " + evnt.ToString());
            ...
            switch (evnt)
            {
                ...
                case EvtType.On_UIElementTouchUp:
                    ...
                    break;
                case EvtType.On_UIElementTouchDown:
                    if (On_UIElementTouchDown != null)
                    {
                        On_UIElementTouchDown(gesture);
                    }
                    break;
            }
            if (UnityEngine.Profiling.Profiler.enabled)
            ...
            // Direct Acces 
            /*int result = findGesturesIndex(gesture.type, gesture.fingerIndex);
            ...
            if (result == -1)
            {
                _currentGestures.Add(gesture.Clone());
        ...
        private int findGesturesIndex(EasyTouch.EvtType type, int fingerIndex)
        {
            for (int i = 0; i < _currentGestures.Count; i++)
            {
                if (_currentGestures[i].type == type && _currentGestures[i].fingerIndex == fingerIndex)
                {
                    return i;
                }
            }
        ...
        private bool IsScreenPositionOverUI(Vector2 position)
        {
            ...
            if (uiEventSystem != null)
            {
                ...
                uiRaycastResultCache = RaycastSystem.CachedRaycastAll(uiEventSystem, uiPointerEventData);
                return uiRaycastResultCache.Count > 0;
                //if (uiRaycastResultCache.Count>0)
                //         {
                //             foreach (var obj in uiRaycastResultCache)
                //             {
                //                 if (!(obj.module is GOGUI.CustomGraphicRaycaster))
                //                 {
                //                     return true;
                //                 }
                //             }
        ...
        public static void ResetAllTouchs()
        {
            if (EasyTouch.instance)
