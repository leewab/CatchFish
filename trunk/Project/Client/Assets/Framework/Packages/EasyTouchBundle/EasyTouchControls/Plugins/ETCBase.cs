...
public abstract class ETCBase : MonoBehaviour {
    ...
    public bool bStopControlPos = false;
	#endregion
    ...
    static public bool AvoidAllocUpdate = true;
	...
	public void SetAnchorPosition(){
		...
		case RectAnchor.BottomLeft:
			...
			this.rectTransform().anchoredPosition = new Vector2( this.rectTransform().sizeDelta.x/2f + _anchorOffet.x, this.rectTransform().sizeDelta.y/2f + _anchorOffet.y);
               // Debug.Log("this.rectTransform().anchoredPosition: x = " + this.rectTransform().anchoredPosition.x + " y = " + this.rectTransform().anchoredPosition.y);
			break;
	...
	protected GameObject GetFirstUIElement( Vector2 position){
		...
		if (uiEventSystem != null){
            ...
            var uiRaycastResultCache = RaycastSystem.CachedRaycastAll(uiEventSystem, uiPointerEventData);
