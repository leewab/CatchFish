...
Shader "CameraFilterPack/AAA_Super_Hexagon" {
	Properties 
	{
		...
		_Progress ("__Progress", Range(0, 1.)) = 0.5
	}
	SubShader 
	{     
...
{
	return lerp(0,0.5,smoothstep(0.0,0.5,+fmod(n+0.5,1.5)));
}
...
float4 frag (v2f i) : COLOR
{
	...
	float mem=1-noise2(_TimeX/2-uv.x/2-uv.y/2);
	...
	//result.a=1-abs(dist2);
	//float newdata=lerp(0,-2,smoothstep(0.0,1,+fmod(_TimeX,1.0)));
	result.a= (_Progress+uv.x+uv.y) * (1-abs(dist2));
